...
if __name__ == "__main__":
	...
	logname = Path(log_dir, f"{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.txt") 
	...
	with torch.no_grad():
		...
		for ii, data_val in enumerate((val_loader), 0):
			...
			with torch.cuda.amp.autocast():
				restored, weightsl1, weights_kl, _ = model_restoration(input_)
	...
	for epoch in range(start_epoch, opt.nepoch + 1):
		...
		for i, data in enumerate(tqdm(train_loader), 0):
			if ii % 200 == 0:
				print(f"Pre-Validation Iteration {ii}")
				...
				with torch.no_grad():
					...
					if psnr_val_rgb > best_psnr:
						...
						torch.save({'epoch': epoch,
									...
									}, model_dir.joinpath("model_best.pth"))
		...
		torch.save({'epoch': epoch,
					'state_dict': model_restoration.state_dict(),
					'optimizer': optimizer.state_dict()
					}, model_dir.joinpath("model_latest.pth"))

		if epoch % opt.checkpoint == 0:
			torch.save({'epoch': epoch,
						'state_dict': model_restoration.state_dict(),
						'optimizer': optimizer.state_dict()
						}, model_dir.joinpath("model_epoch_{}.pth".format(epoch)))
	print("Now time is : ", datetime.datetime.now().isoformat())
