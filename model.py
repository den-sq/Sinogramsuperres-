...
class ConvProjection(nn.Module):
	...
	def forward(self, x, attn_kv=None):
		b, n, c, h = * x.shape, self.heads 	# noqa: F841
...
class WindowAttention(nn.Module):
	...
	def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True,
...
class Upsample(nn.Module):
	...
	def __init__(self, in_channel, out_channel):
		...
		# )
		self.deconv = nn.Sequential(nn.Upsample(scale_factor=2, mode='bicubic', align_corners=True),
...
class BasicUformerLayer(nn.Module):
	...
	def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
		...
		if shift_flag:
			...
			self.blocks = nn.ModuleList([LeWinTransformerBlock(
				dim=dim, input_resolution=input_resolution, num_heads=num_heads, win_size=win_size, shift_size=0,
				mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
				drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
				norm_layer=norm_layer, token_projection=token_projection, token_mlp=token_mlp, modulator=modulator,
...
class L1SpatialAttention(nn.Module):
	def __init__(self):
		super(L1SpatialAttention, self).__init__()
...
class KLSpatialAttention_row(nn.Module):
	def __init__(self, image_size):
		super(KLSpatialAttention_row, self).__init__()
		self.conv = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)
		self.sigmoid = nn.Sigmoid()
		self.line = nn.Linear(image_size, 1)

	def forward(self, x):
		klr = x.clone()
		klr = self.line(klr)
		avg_pool = torch.mean(klr, dim=1, keepdim=True)
		max_pool, _ = torch.max(klr, dim=1, keepdim=True)
		attention = torch.cat([avg_pool, max_pool], dim=1)
		attention = self.conv(attention)
		attention = self.sigmoid(attention)
		out = attention
...
class KLSpatialAttention_col(nn.Module):
	def __init__(self, image_size):
		super(KLSpatialAttention_col, self).__init__()
		self.conv = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)
		self.sigmoid = nn.Sigmoid()
		self.line = nn.Linear(image_size, 1)
	...
	def forward(self, x):
		klc = x.clone()
		klc = self.line(klc)
		avg_pool = torch.mean(klc, dim=1, keepdim=True)
		max_pool, _ = torch.max(klc, dim=1, keepdim=True)
		attention = torch.cat([avg_pool, max_pool], dim=1)
		attention = self.conv(attention)
		attention = self.sigmoid(attention)
		out = attention
...
class Uformer(nn.Module):
	def __init__(self, img_size=256, in_chans=1, dd_in=1,
		...
		self.dd_in = dd_in
		self.l1_attn = L1SpatialAttention()
		self.klr_attn = KLSpatialAttention_row(img_size)
		self.klc_attn = KLSpatialAttention_col(img_size)
	...
	def forward(self, x, mask=None):
		# Input Projection
		l1_attn = self.l1_attn(x)
		klc_attn = self.klr_attn(x)
		klr_attn = self.klc_attn(x)
		...
		# print(y)
		return y, l1_attn, klc_attn, klr_attn
...
# 	print('# model_restoration parameters: %.2f M' % (sum(param.numel() for param in model_restoration.parameters()) / 1e6)) # noqa:E501
